...
class TransformerAutoencoder(nn.Module):
    ...
    def forward(self, src, tgt, src_key_padding_mask=None, tgt_mask=None, tgt_key_padding_mask=None):
        ...
        src_emb = self.pos_encoder(self.embedding(src)).transpose(0, 1) # (seq_len, batch, d_model)
        tgt_emb = self.pos_encoder(self.embedding(tgt)).transpose(0, 1)
        memory = self.encoder(src_emb, src_key_padding_mask=src_key_padding_mask) # encode
