...
def main():
    tokenizer = tokenize("data/EN-DE.txt", 600)
    tokenizer.save("out/tokenizer.json")
    orig = "Das gesetz ist sehr wichtig"
    res = tokenizer.encode("Das gesetz ist sehr wichtig")
    unconverted = tokenizer.decode(res.ids)
...
def use_prev():
    ...
    decoded_text = tokenizer.decode(input_ids.ids, skip_special_tokens=True)
    print(input_ids.tokens)
    print(decoded_text)
    print (f"orig:{orig} \n res:{res} \n unconverted: {unconverted}")

def use_splitter():
    sentence_splitter("data/EN-DE.txt", "data/out.txt")
